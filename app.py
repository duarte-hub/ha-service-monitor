...
def api_update_z2m():
    ...
    def _sup(ws, msg_id, method, endpoint, payload=None):
    ...
    def _do_update():
        ...
        try:
            ...
            mid += 1
            saved_options = info.get("options") or info.get("data", {}).get("options")
            if not saved_options:
                raise RuntimeError(f"Could not read addon options from info response: {info}")
            ...
            _sup(ws, mid, "POST", f"/store/addons/{Z2M_EDGE_SLUG}/install")
            mid += 1

            # Step 5 — wait for addon to reach state "stopped" or "started"
            _set("running", "Waiting for addon to finish installing…")
            for attempt in range(60):
                time.sleep(5)
                try:
                    chk = _sup(ws, mid, "GET", f"/addons/{Z2M_EDGE_SLUG}/info")
                    mid += 1
                    addon_state = chk.get("state") or chk.get("data", {}).get("state", "")
                    log.info("Z2M update: addon state after install = %s", addon_state)
                    if addon_state in ("stopped", "started", "running"):
                        break
                except Exception as e:
                    mid += 1
                    log.warning("Z2M update: poll attempt %d failed: %s", attempt, e)
                _set("running", f"Waiting for install… ({(attempt + 1) * 5}s)")
            else:
                raise RuntimeError("Timed out waiting for addon to install")

            # Step 6 — restore config options
            _set("running", "Restoring config options…")
            _sup(ws, mid, "POST", f"/addons/{Z2M_EDGE_SLUG}/options", {"options": saved_options})
            mid += 1
